...
class_cl=['ProbabilisticMethods','Theory','ReinforcementLearning','CaseBased','NeuralNetworks','GeneticAlgorithms'
    ,'RuleLearning']
...
for jj in range(1):
    ...
    valid_data = []
    # 每个类按比例抽取
    my_matrix = np.loadtxt(open("cora_cite_class.txt"), dtype=str, skiprows=0)
    X, y = my_matrix[:, :-1], my_matrix[:, -1]
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.9)  # 测试集比例哦
